...
class KalmanFilter:
    ...
    def compute_association_probabilities(self, S):
        ...
        for i in range(num_measurements):
            # Adjust the dimension of self.H[i] to match inv_S
            H_i = self.H[i]
            d = np.dot(np.dot((H_i - np.dot(H_i, self.x)).T, inv_S), (H_i - np.dot(H_i, self.x)))
            association_probabilities[i] = np.exp(-0.5 * d) / ((2 * np.pi) ** (self.H.shape[0] / 2) * np.sqrt(det_S))
        return association_probabilities

def main():
    # Read data from CSV file, read specified columns
    data = pd.read_csv("test.csv", usecols=[6, 7, 8, 9, 10, 11, 12, 13])

    # Extract time and measurements into separate arrays
    time = data.iloc[:, -1].values
    measurements = data.iloc[:, :-1].values

    # Define state transition matrix
    F = np.eye(8)  # Assume constant velocity model for simplicity

    # Define measurement matrix (7x8 matrix)
    H = np.eye(7, 8)  # Identity matrix since measurement directly reflects state

    # Define process noise covariance matrix
    Q = np.eye(8) * 0.1  # Process noise covariance

    # Define measurement noise covariance matrix
    R = np.eye(7) * 0.01  # Measurement noise covariance, adjusted variance
